...
const applicationName = options.wallet ? 'Dogethereum Wallet' : 'Dogethereum Mist';
...
gulp.task('bundling-interface', cb => {
  ...
  if (type === 'wallet') {
    if (options.walletSource === 'local') {
      ...
      console.log(
        `Pulling https://github.com/dogethereumproject/meteor-dapp-wallet/tree/${
          options.walletSource
        } "${options.walletSource}" branch...`
      );
      bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/dogethereumproject/meteor-dapp-wallet.git \
...
gulp.task('build-dist', cb => {
  const appPackageJson = _.extend({}, require('../package.json'), {
    ...
    homepage: 'https://github.com/dogethereumproject/mist',
    build: {
